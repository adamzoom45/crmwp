...
class AKPP_Avito_Cron {
    ...
    public function __construct() {
        ...
        // Очистка при деактивации (опционально, можно вынести в register_deactivation_hook)
    ...
    public function schedule_sync_event() {
        // Проверяем, включена ли автосинхронизация в настройках
    ...
    public function run_sync() {
        // Блокировка одновременного выполнения (transient lock на 5 минут)
        if (get_transient('akpp_avito_sync_lock')) {
            return;
        }
        set_transient('akpp_avito_sync_lock', true, 5 * MINUTE_IN_SECONDS);
        ...
        try {
            ...
            // 1. Получаем последние 30 диалогов из Авито
            ...
            if (empty($dialogs_response['dialogs'])) {
            ...
            foreach ($dialogs_response['dialogs'] as $avito_dialog) {
                $avito_dialog_id = intval($avito_dialog['id']);
                $last_message    = $avito_dialog['last_message'] ?? null;
                
                // Проверяем, есть ли диалог у нас в БД
                $local_dialog = $wpdb->get_row($wpdb->prepare(
                    "SELECT id, last_message_id FROM $dialogs_table WHERE avito_dialog_id = %d",
                    $avito_dialog_id
                ), ARRAY_A);
                ...
                if ($last_message && (!$local_dialog || intval($last_message['id']) > intval($local_dialog['last_message_id']))) {
                    $has_new_messages = true;
                }

                // 3. Если диалог новый или есть новые сообщения, забираем их
                if ($is_new_dialog || $has_new_messages) {
                    $messages_response = $api->make_request("/messages/1/dialogs/{$avito_dialog_id}/messages?limit=10", 'GET');
                    
                    if (!is_wp_error($messages_response) && !empty($messages_response['messages'])) {
                        foreach ($messages_response['messages'] as $msg) {
                            $avito_msg_id = intval($msg['id']);
                            ...
                            if (!$msg_exists) {
                                $direction = (intval($msg['author_id']) > 0) ? 'incoming' : 'outgoing';
                                
                                $wpdb->insert(
                                    $messages_table,
                                    [
                                        'avito_message_id' => $avito_msg_id,
                                        'dialog_id'        => $local_dialog ? $local_dialog['id'] : 0, // Будет обновлено ниже
                                        'author_id'        => intval($msg['author_id']),
                ...
                // 4. Обновляем или создаём запись диалога в нашей БД
                ...
                $item_title  = sanitize_text_field($avito_dialog['item']['title'] ?? '');
                $item_price  = intval($avito_dialog['item']['price'] ?? 0);
                ...
                if ($last_message) {
                    ...
                    $dialog_data['last_message_date'] = sanitize_text_field($last_message['created_at']);
                    $dialog_data['last_message_direction'] = (intval($last_message['author_id']) > 0) ? 'incoming' : 'outgoing';
                }
                ...
                // 5. Пересчёт непрочитанных сообщений для этого диалога
                if ($dialog_db_id) {
                    // Сначала привязываем все сообщения без dialog_id к этому диалогу
                    $wpdb->update(
                        $messages_table,
                        ['dialog_id' => $dialog_db_id],
                        ['avito_message_id' => $avito_dialog_id], // Упрощённо, в реальности лучше по author_id и времени
                        ['%d'],
                        ['%d']
                    );
            ...
            error_log(sprintf('[AKPP Avito Cron] Синхронизация завершена. Обработано сообщений: %d', $synced_count));
